...
#	L0="\033[1m"  # Bold
	...
	def __substituteVarsInSection_analysis(self):
	...
	def __getAppBatchScript(self,whichdataset,p):
				...
				for k in self.a_apps.keys():
					if which==k:
						...
						if  mybatch == None or mybatch['name'] == "NONE":
							...
							self.log.warning("Warning","No batch system specified for app: " + self.log.bold(which) )
							...
							self.log.plain("Using " + self.log.bold(str(mybatch['name'])) + " for app: " + self.log.bold(which) )
							#print "Submission command: "
							self.log.log("Submission command:")
							Log.Level = 1
						...
						for   dataset in self.a_apps[which]:
							nprocs = str(dataset['numprocs']).split(',')
							if len(nprocs) == 0:
								#print KABS.L2 + "No procs found ... dataset " + str( dataset['name'] )  +" skipped"	+ KABS.LE
								self.log.warning("Warning","No procs found ... dataset " + self.log.bold(str( dataset['name'] ))  +" skipped")
								continue
						
							for p in nprocs:
								#print "dataset: "+KABS.L1 + str( dataset['name'] ) + KABS.LE  + " with " + KABS.L1 + p + KABS.LE + " procs"
								...
								data = self.__getAppBatchScript(dataset,p,)
								print data
								print "-------------------------------------------------"									
						break;	
	...
	def __printSynthInfo(self):
		#print "\t"+KABS.L0+"Synthetic:"+KABS.LE
		Log.Level = 1
		self.log.log("Synthetic:")

	
	def __printNetInfo(self):
		#print "\t"+KABS.L0+"Networks:"+KABS.LE
		Log.Level = 1
		self.log.log("Networks:")

	
	def __printMathlibInfo(self):
		#print "\t"+KABS.L0+"Math Libs:"+KABS.LE
		Log.Level = 1
		self.log.log("Math Libraries:")

	
	def __printAcceptanceInfo(self):
#		print "\t"+KABS.L0+"Acceptance:"+KABS.LE								
		Log.Level = 1
	...
	def __analysisApp(self,name):
		...
		analysisd = self.output_dir + "/analysis/"+ name + "/"
		runsd = self.output_dir + "/runs/"+ name + "/"
	...
	def __runApp(self,whichapp):
		...
		for dataset in app:
			...
			nprocs = str(dataset['numprocs']).split(',')
			if len(nprocs) == 0:
				#print L2 + "No procs found ... dataset " + str( dataset['name'] )  +" skipped"	+ KABS.LE
				self.log.warning("Warning","No procs found ... dataset " + self.log.bold( str( dataset['name'] ))  + " skipped")
				continue
...
set grid polar
	...
	def analysis(self,item=None,name=None):
		...
		if item == None: # means analyze everything	
			...
			#print "***********************************************"
			self.log.plain("***********************************************")
			#print  "*** "+KABS.L0+" KaBS analysis stage for selected App: " + KABS.LE+" ***"	
			...
			if self.a_apps.keys().count(name) == 0:
				self.log.plain(  "Application " +  self.log.bold( name ) + " not found or not active"	)
				return
	...
	def run(self,item=None,name=None):
		...
		if item == None: # means run everything	
			...
			#print "********************************************"
			self.log.plain("********************************************")
			#print  "*** "+KABS.L0+" Running KaBS for selected App:      " + KABS.LE + "***"
	...
	def printConf(self, item=None, name=None ):
		...
		#print  "*************************************************************"
		self.log.log("*************************************************************")
		#print  "*** "+KABS.L0+" Current configuration for the KAUST Benchmark Suite "+KABS.LE+" ***"	
		...
		if item == None: # means show the global configuration	
			...
			#print  "*** "+KABS.L0+" Showing configuration for selected App "+ KABS.LE+" ***"
			...
			#print  "*** "+KABS.L0+" Showing configuration for Filesystems "+KABS.LE+" ***"	
			self.log.plain("\n***  Showing configuration for Filesystems  ***\n"	)
			self.__printFSInfo()
		elif item == 'n':
			#print  "*** "+KABS.L0+" Showing configuration for Networks "+KABS.LE+" ***"	
			self.log.plain("\n***  Showing configuration for Networks  ***\n")
			self.__printNetInfo()
		elif item == 's':
			#print  "*** "+KABS.L0+" Showing configuration for Synthetic benchmarks "+KABS.LE+" ***"	
			self.log.plain("\n***  Showing configuration for Synthetic benchmarks  ***\n")	
			self.__printSynthInfo()
		elif item == 'm':
			#print  "*** "+KABS.L0+" Showing configuration for Math Libs "+KABS.LE+" ***"	
			self.log.plain("\n***  Showing configuration for Math Libs  ***\n"	)
			self.__printMathlibInfo()
		elif item == 'x':
			#print  "*** "+KABS.L0+" Showing configuration for the Acceptance benchmark "+KABS.LE+" ***"	
	...
	for attr, value in opts.__dict__.iteritems():		
		if ( ( attr=='d' or attr=='r' or attr=='p' or attr=='c' ) and value==True ) or (attr=='k' and value!=None) :
			...
			if (attr=='k' and value!=None):	
				itemname = value
		if  ( ( attr=='a'  or  attr=='n' or  attr=='f' or  attr=='m'  ) and value!= None) or ( ( attr=='s' or attr=='x' ) and value==True):
			# got selector
			selector = attr
			itemname = value
